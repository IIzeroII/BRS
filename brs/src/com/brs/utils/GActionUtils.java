...
/**
 * 封装关于原生HTTP的对象的操作：如对request的获取与赋值
 *  
 * 注意：请不要与项目其他工具包（如GUtils等）发生关系，保持工具包的纯洁性
 *
 */
public class GActionUtils {
	...
	public static void writeImgByPath(String imgPath, String imgName,
		...
		try {
			...
			imgFile = new File(imgPath, defaultImgPath);
			e.printStackTrace();
		} finally {
			writeImg(imgFile);
		}
	...
	public static Boolean setRequestAttr(String name, Object o) {
		try {
			//ServletActionContext继承了ActionContext,它还提供了直接访问Servlet的相关对象的功能
	...
	public static <T> T getRequestParam(String name) {
		...
		try {
			return (T) ServletActionContext.getRequest().getParameter(name);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
